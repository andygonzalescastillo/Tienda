...
import { useSessionManager } from '@/user/hooks/useSessionMutations';
...
export const UserMenu = () => {
    ...
    const iniciales = usuario?.nombre?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) || 'U';
    const gradiente = getGradient(usuario?.nombre || 'U');

    const handleAction = async (action: 'config' | 'logout') => {
        if (action === 'config') return navigate(ROUTES.USER.CONFIGURATION);
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-64">
                ...
                <DropdownMenuItem onClick={() => navigate(ROUTES.USER.SESSIONS)}>
